...
CREATE TABLE IF NOT EXISTS user_behavior (
    ...
    user_id VARCHAR(128) NOT NULL COMMENT '访客标识',
    event_type VARCHAR(32) NOT NULL COMMENT '事件类型（保留 SDK 上报值）',
    page_url VARCHAR(512) COMMENT '页面URL',
    device_type VARCHAR(20) COMMENT '设备类型：PC-电脑，MOBILE-手机，TABLET-平板',
    region VARCHAR(32) COMMENT '地域：华北/华东/华南/华中/西南/西北/东北',
...
CREATE TABLE IF NOT EXISTS behavior_stats (
    ...
    region VARCHAR(32) NOT NULL DEFAULT '' COMMENT '地域',
    create_time DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    INDEX idx_stat_time (stat_time),
    INDEX idx_stat_type (stat_type),
    UNIQUE KEY uk_behavior_stats_scope (stat_time, stat_type, region)
